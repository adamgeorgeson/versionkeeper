...
describe ReleaseNote do
  ...
  it "returns a message if no content found for given paramaters" do
  ...
  describe :build_release_notes do
    ...
    xit 'returns message if no release notes found for a release' do
      # subject.build_release_notes(release).stub(:release_notes).and_return(nil)
      subject.stub(:retrieve_release_notes)
      expect(subject.build_release_notes(release)).to eq 'There are no release notes ready yet. Try again later.'
    end

    xit 'returns stored version of release notes if present AND release has status of Production' do
    ...
    it 'saves release notes to database for current release' do
      expect {subject.build_release_notes(release)}.to change(ReleaseNote, :count).by 1
    end
  end
end
